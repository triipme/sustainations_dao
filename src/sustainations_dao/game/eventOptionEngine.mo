...
module EventOptionEngine {
  ...
  public func create(eventOption : Types.EventOption, state : State.State) {
    state.questEngine.eventOptions.put(eventOption.id, getData(eventOption));
  };

  public func update(eventOption : Types.EventOption, state : State.State) {
    let updated = state.questEngine.eventOptions.replace(eventOption.id, getData(eventOption));
  };
}