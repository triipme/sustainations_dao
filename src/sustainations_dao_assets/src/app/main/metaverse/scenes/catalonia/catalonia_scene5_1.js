...
export default class catalonia_scene5_1 extends BaseScene {
  ...
  preload() {
    ...
    this.characterBefore = this.useUsableItemScene(this.isUsedUsableItem, "e13");
  ...
  async create() {
    ...
    for (let x = -50; x < 3700; x += 4) {
      platforms.create(x, 635, "ground").refreshBody();
    }
    this.physics.add.collider(this.player, platforms);

    this.createUIElements();
    this.defineCamera(3631, gameConfig.scale.height);
  ...
  update() {
    ...
    if (this.player.x > 3631) {
      this.ingameSound.stop();
      this.sfx_char_footstep.stop();
      this.scene.start('catalonia_scene5_2', { isUsedPotion: this.isUsedPotion, isUsedUsableItem: this.isUsedUsableItem });
    }

    if (this.player.x > 2900 && this.isInteracted == false) {
