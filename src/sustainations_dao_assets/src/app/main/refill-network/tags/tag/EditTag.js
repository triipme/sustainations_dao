...
const EditTag = () => {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const result = await user.actor.getRBTag(tagId)
        if ('ok' in result) {
          reset({
            name: result.ok.name,
          });
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const result = await user.actor.updateRBTag(tagId, data.name);
      if ("ok" in result) {
  ...
  return (
    <FusePageCarded
      header={<FormHeader actionText="Edit Tag" backLink="/refill-network/tags" />}
